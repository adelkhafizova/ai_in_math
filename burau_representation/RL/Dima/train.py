...
from burau_representation.RL.Dima.model import BurauDQN, BurauDQNLSTM
...
def train_fc(
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if episode % 1000 == 0:
            ...
            plot_word_power_range(eval_word, eval_ranges, file_name=f'fc_mod{modulo}_length{max_steps}_power_range_episode_{episode}.png')
        ...
        if episode % 10000 == 0:
            plot_epochs_avg_loss(avg_loss_history, filename=f'fc_mod{modulo}_length{max_steps}_avg_loss_up_to_{episode}.png')

    plot_epochs_avg_loss(avg_loss_history, filename=f'fc_mod{modulo}_length{max_steps}_avg_loss.png')
...
def train_lstm(
    ...
    # Networks and optimizer
    policy_net = BurauDQNLSTM(input_size=1).to(device)
    target_net = BurauDQNLSTM(input_size=1).to(device)

    # policy_net.load_state_dict(torch.load("lstm_mod2_length34_policy_net_weights.pth"))
    # target_net.load_state_dict(torch.load("lstm_mod2_length34_policy_net_weights.pth"))
    ...
    for episode in range(1, num_episodes + 1):
            ...
            if raw_r != 0.0:
                if raw_r > 0:
                    ...
                    if identity_word not in identities:
                        ...
                        with open('identity.txt', 'a') as f:
                            f.write(identity_word + '\n')
                            torch.save(policy_net.state_dict(), f"lstm_mod{modulo}_length{max_steps}_policy_net_weights.pth")
                reward = raw_r
            else:
                if curr_max < prev_max:
                    reward = 1.0
        ...
        if episode % 1000 == 0:
            ...
            eval_range = largest_power_range(eval_env.product)
            eval_info  = f", EvalWord={eval_word}, EvalRange={eval_range}"
            plot_word_power_range(eval_word, eval_ranges, file_name=f'lstm_mod{modulo}_length{max_steps}_power_range_episode_{episode}.png')
        ...
        if episode % 10000 == 0:
            plot_epochs_avg_loss(avg_loss_history, filename=f'lstm_mod{modulo}_length{max_steps}_avg_loss_up_to_{episode}.png')

    plot_epochs_avg_loss(avg_loss_history, filename=f'lstm_mod{modulo}_length{max_steps}_avg_loss.png')